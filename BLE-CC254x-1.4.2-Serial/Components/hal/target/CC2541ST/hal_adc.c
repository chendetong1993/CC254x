...
bool HalAdcRead (uint8 channel_resolution, uint8* status);
 ...
 **************************************************************************************************/
bool HalAdcRead (uint8 channel_resolution, uint8* status){
  uint8 Chan = channel_resolution >> 4;
  uint8 Res = channel_resolution & 0x0F;
  if((Chan < HAL_ADC_CHANNEL_NUM) && (Res < HAL_ADC_RESOLUTION_NUM)){
    *status = (uint8)((float)HalAdcReadCore(Chan, Res) / 2048 * 255);
    return TRUE;
  }
  return FALSE;
}
